...
// Obtain the key of the route reuse cache, in the form of key+param: login{name:xxx}
...
const fnGetReuseStrategyKeyFn = function getKey(route: ActivatedRouteSnapshot): string {
  ...
  // is a query parameter, and has parameters
  if (Object.keys(route.queryParams).length > 0) {
    return configKey + JSON.stringify(route.queryParams);
  } else if (Object.keys(route.params).length > 0) {
    // is a path parameter, and has parameters
...
// weak-theme converted to weakTheme
