...
export class ChartComponent implements AfterViewInit, OnInit {
  ...
  private removeDuplicatesTootip(data: { timeSpan: number; value: number }[]): {
    [key: string]: string;
  } {
    if (data === null) {
      return null;
      ...
      return data.reduce((acc, item) => {
        ...
        if (
          !(timeKey in acc) ||
          item.value >
            parseFloat(acc[timeKey].split(' ')[0]) *
              Math.pow(1024, this.getExponent(acc[timeKey]))
        ) {
  ...
  private getExponent(value: string): number {
  ...
  private removeDuplicatesNumberObject(
    data: { timeSpan: number; value: number }[]
  ): { [key: string]: number } {
    return data.reduce((acc, item) => {
      const timeKey = this.transformTooltip(item.timeSpan);
    ...
    this.chartStorageUse = new Chart({
      ...
      xAxis: {
        ...
        labels: {
          formatter: function () {
            return `${
              this.value.toString().split(' ')[1] === undefined
                ? this.value.toString()
                : this.value.toString().split(' ')[1]
            }`;
          },
        },
      },
      ...
      tooltip: {
        formatter: function () {
          return `<b>${this.x}</b><br/>${this.series.name}: ${
            uniqueData[labels[this.point.index]]
          }`;
        },
      },
      series: [
