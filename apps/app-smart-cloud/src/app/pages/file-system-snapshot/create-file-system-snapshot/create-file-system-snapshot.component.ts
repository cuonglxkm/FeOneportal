...
import { FormCreateFileSystemSnapShot, OrderCreateFileSystemSnapshot } from 'src/app/shared/models/filesystem-snapshot';
...
export class CreateFileSystemSnapshotComponent implements OnInit{
  ...
  formCreate: OrderCreateFileSystemSnapshot = new OrderCreateFileSystemSnapshot();
  ...
  caculator(event) {
    this.expiredDate = addDays(this.dateString, 30 * this.form.get('time').value);
  }
  ...
  fileSystemSnapshotInit(){
      ...
      this.formCreate.actionType = ServiceActionType.CREATE;
      this.formCreate.serviceInstanceId = 0;
      this.formCreate.createDate = this.typeVpc === 0 ? this.dateString : new Date()
      this.formCreate.expireDate = this.typeVpc === 0 ? this.expiredDate : new Date()
      this.formCreate.createDateInContract = null;
      this.formCreate.saleDept = null;
      this.formCreate.saleDeptCode = null;
      this.formCreate.contactPersonEmail = null;
