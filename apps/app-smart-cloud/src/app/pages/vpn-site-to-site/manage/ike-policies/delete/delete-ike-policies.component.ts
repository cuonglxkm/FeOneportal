  ...
  handleOk() {
    ...
    if(this.validateForm.valid) {
      if(this.nameIkePolicy.includes(this.validateForm.controls.name.value)){
        this.ikePolicyService.deleteIkePolicy(formDelete).subscribe(data => {
          if(data) {
            this.isVisible = false
            this.isLoading =  false
            this.notification.success('Thành công', 'Xoá Ike Policy thành công')
            this.onOk.emit(data)
          }
        }, error => {
          this.isVisible = false
          ...
          console.log("error xoa ike",error);
        })
      }
    }
  }
